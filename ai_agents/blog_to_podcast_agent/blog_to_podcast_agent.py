    ...
    if not url.strip():
        ...
        with st.spinner("Scraping blog and generating podcast..."):
            try:
                ...
                # (tạo agent cho việc thu thập dữ liệu và tóm tắt dữ liệu)
                # FirecrawlTools là một công cụ cho việc thu thập dữ liệu từ web
                agent = Agent(
                ...
                # Get summary
                # RunOutput là một đối tượng trả về từ hàm run của agent, bao gồm:
                # content là nội dung câu trả lời của agent
                # tools là các công cụ mà agent đã sử dụng trong quá trình chạy
                # metrics là các chỉ số performance của agent
